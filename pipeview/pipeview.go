...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	// height = clamp(height+stride*2, 0, len(g.dump)-screenHeight*screenWidth*4)
...
func main() {
	...
	go func(bufIO *bufio.Reader) {
		for {
			...
			if err != nil {
				// g.dump = append(g.dump, make([]byte,stride*screenHeight/2)...)
				break
				// log.Fatal(err)
			}
		...
		// i := 0
		// j := 0
		// fmt.Println("here we go boys", side, side*side, len(g.dump)-screenHeight*screenWidth*4)
		// for idx := screenHeight * screenWidth * 4; idx < len(g.dump); idx += 4 {
		// 	img.Set(i, j, color.RGBA{g.dump[idx+0], g.dump[idx+1], g.dump[idx+2], g.dump[idx+3]})
		// 	i++
		// 	if i >= side {
		// 		i = 0
		// 		j++
